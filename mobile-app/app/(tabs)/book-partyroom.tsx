...
export default function BookPartyRoomScreen() {
    ...
    const partyRoomBookings = useMemo(
        () =>
            [...bookings]
                .filter((booking) => booking.date && booking.facilityName?.toLowerCase().includes('fest'))
                .sort((left, right) => {
            ...
            <View style={styles.container}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                    <View style={styles.section}>
                        ...
                        {partyRoomBookings.length > 0 ? (
                            partyRoomBookings.map((booking) => (
                                <View key={booking.bookingId} style={styles.bookingCard}>
                                    <View style={styles.bookingCardContent}>
                                        <Text style={styles.bookingTitle}>Festsal</Text>
                                        <Text style={styles.bookingMeta}>{formatDisplayDate(booking.date ?? '')}</Text>
                                    </View>
...
const styles = StyleSheet.create({
    ...
    confirmButtonDisabled: {backgroundColor: '#98A2B3'},
    confirmButtonText: {fontSize: 18, fontWeight: '800', color: '#FFFFFF'},
});
